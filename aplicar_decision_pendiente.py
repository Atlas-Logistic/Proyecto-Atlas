...
def main():
    ...
    # --rut-manual (opcional).
    # Bloque CORRECCIÓN HUMANA TRANSVERSAL: --cliente-correccion-manual es
    # opcional y sólo aplica a CLIENTE_CANDIDATO/CONFIRMAR -- un nombre de
    # OTRO cliente ya existente en el catálogo que reemplaza al candidato
    # sugerido (aplicar_decision_obra ya valida que exista y sea único).
    parser=argparse.ArgumentParser(); parser.add_argument("--raiz-atlas",required=True); parser.add_argument("--decision-id",required=True); parser.add_argument("--accion",choices=("REGISTRAR","NO_REGISTRAR","CONFIRMAR","NO_CONFIRMAR","POSPONER","CONFIRMAR_PLANTA","SELECCIONAR_OTRA_PLANTA","NO_PUEDO_DETERMINAR","USAR_PATENTE_EXISTENTE","SELECCIONAR_OTRA_PATENTE","CONFIRMAR_ALIAS","RECHAZAR","REGISTRAR_DIRECCION","REGISTRAR_CLIENTE_MANUAL"),required=True); parser.add_argument("--tipo-vehiculo",choices=("TRACTO","CARRO","CAMION_RIGIDO")); parser.add_argument("--planta-id-elegida"); parser.add_argument("--patente-elegida"); parser.add_argument("--motivo-rechazo"); parser.add_argument("--direccion-manual"); parser.add_argument("--comuna-manual"); parser.add_argument("--razon-social-manual"); parser.add_argument("--rut-manual"); parser.add_argument("--nombre-obra-manual"); parser.add_argument("--cliente-correccion-manual"); parser.add_argument("--rut-chofer-elegido")
    args=parser.parse_args()
    argumentos=dict(raiz_atlas=args.raiz_atlas,decision_id=args.decision_id,accion=args.accion,tipo_vehiculo=args.tipo_vehiculo,planta_id_elegida=args.planta_id_elegida,patente_elegida=args.patente_elegida,motivo_rechazo=args.motivo_rechazo,direccion_manual=args.direccion_manual,comuna_manual=args.comuna_manual,razon_social_manual=args.razon_social_manual,rut_manual=args.rut_manual,nombre_obra_manual=args.nombre_obra_manual,cliente_correccion_manual=args.cliente_correccion_manual,rut_chofer_elegido=args.rut_chofer_elegido)
    try:
