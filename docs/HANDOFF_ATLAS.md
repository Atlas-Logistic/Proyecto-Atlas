...
**Arquitectura dev actual (a reemplazar).** `iPhone/PWA → dev-server Node 8443 (HTTPS autofirmado, IP LAN, en el PC de Javier) → proxy /api/mobile/* → Motor 127.0.0.1:8765`. Con el PC apagado el chofer no puede autenticarse ni subir. Repos: PWA en `Atlas-Conductores-Mobile` (estática, sin build; `scripts/dev-server.js`), Motor en este repo (`servidor_mobile.py`, `atlas_core/mobile.py`).

**Arquitectura aprobada M1.** `PWA (sin cambios de código) → intake HTTPS 24/7 (cloud) → almacenamiento durable cloud`. El intake **no** procesa OCR/Motor: sólo `POST /api/mobile/login` + `POST /api/mobile/envios` (contrato intacto), valida token + payload, y persiste el envío (foto + `envio.json`) de forma atómica. El **Motor local drena después** por HTTPS (pull) cuando el PC se enciende y procesa localmente con el pipeline existente. Motor apagado = el backlog se acumula durable en el cloud y se drena en orden al encender.

- **Hosting recomendado:** VPS mínimo (Hetzner CX22 / DigitalOcean 1 GB) + Caddy (TLS Let's Encrypt automático, sirve también la PWA + proxy reverso al intake). Reusa el código Python. Alternativa: Cloudflare Pages + Worker + R2 (reescribe el intake en JS, menos operación). Coste piloto ≈ US$6–8/mes (VPS + dominio + object storage; TLS gratis); Cloudflare ≈ US$0–5.
- **Dominio/TLS:** subdominio dedicado (p. ej. `conductores.<dominio>`), A/AAAA al host, Caddy emite/renueva solo. Fin del autofirmado y del diálogo de Safari.

**Reutilizable tal cual** (verificado leyendo ambos repos): PWA completa (`storage.js`, `sync.js`, `sync-core.js`, `service-worker.js`, `camera.js`, `auth.js`, `app.js`, `index.html`, manifest -- incluyendo cola IndexedDB, Background Sync, reintento 30 s, banner de auto-update por `/version.json`); `AutenticadorMobile`, `hash_password`/`verificar_password`, `usuarios_mobile.json` (en `catalogos_privados/`), `crear_usuario_mobile.py`; `RepositorioEnviosMobile` (recibir/cargar/guardar); `crear_servidor` + handlers `/login` y `/envios`, `_multipart`, bandera `procesar` (ya existe); pipeline local (`procesar_envio_mobile`, `_procesar_y_revalidar`, `reprocesar_envio_mobile_persistido`); `dev-server.js` (se queda para pruebas E2E locales; `ATLAS_MOBILE_API_PROXY` ya configurable). Contrato de payload y modelo de idempotencia por `envio_id`.

**Contrato Mobile (intacto en M1).** `POST /api/mobile/login` JSON `{usuario,password}` → `{token,chofer_id}`. `POST /api/mobile/envios` multipart, `Authorization: Bearer <token>`, campos `envio_id` (UUID del dispositivo, estable en todos los reintentos = clave de idempotencia), `schema_version=1`, `capturado_en` (ISO-8601 Z), `tipo_novedad` (5 valores o vacío), `guia_firmada_correo` (`"true"`/`"false"`), `planta_origen_informada` (`AZA_COLINA`/`AZA_RENCA`), `lote_id` (opcional), `imagen`. Respuesta `202 {resultado:"ACEPTADO",envio_id,estado,duplicado}`. Límites: `MAX_PAYLOAD_BYTES=30 MiB` (servidor), `MAX_IMAGEN_BYTES=28 MiB` (core), MIME JPEG/PNG/WebP, `_ID_SEGURO` regex para `envio_id`, filename ignorado, escritura `fsync`+`os.replace`. Zona canónica: `operacion/mobile/envios/<envio_id>/original.<ext>` + `envio.json`; la foto nunca se sustituye; fast-path idempotente: si `envio.json` existe → devuelve `(registro, False)` sin lock ni reescritura.

**Autenticación -- qué se conserva / cambia.** Se conserva: contrato `/login`, `AutenticadorMobile` (token HMAC-SHA256 `cuerpo.firma` base64url, TTL 86400 s, secreto ≥24 chars en `ATLAS_MOBILE_TOKEN_SECRET`), `usuarios_mobile.json` con PBKDF2 200k. Cambia para el piloto: secreto generado fresco y sólo en el host (no reusar el de dev); `ATLAS_MOBILE_ALLOWED_ORIGIN` fijado al dominio (no `*`); rate-limit en `/login`; TTL de token subido (p. ej. 30 días) -- **sin refresh ni revocación individual** (aceptado; única palanca = rotar el secreto → todos re-login). Añadir un chofer = re-correr `crear_usuario_mobile.py` + re-desplegar `usuarios_mobile.json` al host.

**Cambios OBLIGATORIOS (5) antes de construir:**
1. **Desacoplar auth/repo/intake de dependencias OCR pesadas.** Verificado: `atlas_core/mobile.py` importa a nivel de módulo (líneas 24-26) `ocr_provider.crear_proveedor_ocr` y `procesamiento_masivo` (`COLUMNAS`, `procesar_archivo`, `escalar_resultado_ia_en_memoria`, `_escribir_filas`); `servidor_mobile.py` importa `revalidacion_documental` en el top. `AutenticadorMobile` y `RepositorioEnviosMobile` viven en `mobile.py` → importar el intake arrastra easyocr/paddle/groq/torch. Fix: lazy-import de todo el pipeline detrás de `procesar=True`, o extraer auth+repo+`_multipart` a un módulo sin dependencias de OCR. **Prerequisito duro: nada de M1 arranca lean sin esto.**
2. **Lifecycle cloud `RECIBIDO`/`DRENADO`, separado del estado operacional.** El `envio.json` del intake es fuente de verdad SÓLO de "recibimos este envío". 2 estados: `RECIBIDO` → `DRENADO`. `GET /pendientes` filtra `RECIBIDO`. No reutilizar `RepositorioEnviosMobile.pendientes()` -- filtra `estado == "REQUIERE_REVISION"` (semántica del Motor). El dataset/Desktop sigue siendo fuente de verdad de "procesado / resultado operacional".
3. **ACK = copia durable local, NO procesamiento.** Drainer: `GET /pendientes` → descarga imagen + `envio.json` → **verifica bytes contra `envio.json.imagen_sha256`** → escribe local con el patrón atómico de `RepositorioEnviosMobile` → **ACK** (→ `DRENADO`, atómico, idempotente; ACK repetido o de id desconocido = 200 no-op). El procesamiento local (OCR/Motor) es un paso posterior separado, con su propio marcador y reintento idempotente; un fallo de procesamiento nunca des-ACKea ni re-descarga.
4. **Token de drenaje independiente + validación/rate-limit/CORS/TLS.** Secreto propio (`ATLAS_MOBILE_DRAIN_TOKEN`, ≥32 bytes aleatorios, distinto del de choferes), `hmac.compare_digest`; endpoints de drenaje responden `404` si el secreto no está seteado; `_ID_SEGURO.fullmatch` en cada parámetro de ruta antes de tocar disco; token de chofer y token de drenaje nunca se aceptan cruzados; TLS-only + HSTS; `ALLOWED_ORIGIN` al dominio.
5. **Backup externo versionado, RPO ≤ 10 min.** Nocturno pierde hasta un día si el disco muere por la tarde. Sync a object store (R2/B2) cada 5–10 min (o push en el `recibir`), **versioning activado**, datos en un volumen de bloque separado con snapshots.

**API de drenaje (nueva, mismo binario, activa sólo con `ATLAS_MOBILE_DRAIN_TOKEN`):** `GET /api/mobile/pendientes` (lista `envio_id` + metadatos en estado `RECIBIDO`), `GET /api/mobile/envios/<id>` (el `envio.json`), `GET /api/mobile/envios/<id>/imagen` (bytes de la foto), `POST /api/mobile/envios/<id>/ack` (→ `DRENADO`). Cliente local: `drenar_envios_mobile.py` (nuevo) + tarea programada de Windows / al arrancar.

**Orden exacto:**
- **M1-A -- Intake lean + hosting.** Cambio 1 (split de dependencias, bloqueante) + flag `--solo-intake` (fija `procesar=False`, storage dir configurable) + Dockerfile + Caddyfile + dominio + TLS + `usuarios_mobile.json` en el host + secretos frescos. *Gate:* `login`+`envios` desde datos móviles con todo Atlas apagado; el envío sobrevive un reinicio del host.
- **M1-B -- Drenaje fiable.** Cambios 2 y 3 + parte de drenaje del cambio 4: lifecycle `RECIBIDO`/`DRENADO`, endpoints de drenaje endurecidos, `drenar_envios_mobile.py` con verificación SHA-256 y ACK-tras-copia, tarea programada, paridad exacta de layout local. *Gate:* Motor 24 h apagado → drena todo el backlog; cada envío queda en `operacion/mobile/envios/<envio_id>/` byte-idéntico a una subida directa; re-descargas por fallo no producen efecto duplicado (idempotencia).
- **M1-C -- Durabilidad, borde y cutover.** Cambio 5 (RPO ≤10 min, versioning, volumen) + resto del cambio 4 (`ALLOWED_ORIGIN`, rate-limit, TTL, cabeceras `Cache-Control: no-store` en `/version.json` y `service-worker.js`) + PWA apuntada al dominio y reinstalada una vez (confirmando que ningún teléfono tiene cola pendiente en el origen LAN) + runbook (rotar secreto, restaurar desde backup, volver al dev-server LAN como puente). *Gate:* se borra el volumen y se restaura cumpliendo el RPO declarado; el banner de auto-update funciona desde el origen nuevo.

**Aclaraciones canónicas (redacción obligatoria en toda doc/PR de M1):**
- **No afirmar "exactly once".** La garantía es **idempotencia / efecto único** aunque haya re-descargas por fallos entre descarga → copia → ACK → procesamiento.
- El paso de la PWA dev (IP LAN) al dominio productivo **puede requerir UN cutover/reinstalación inicial** por cambio de origen (service worker + IndexedDB nuevos en el dominio).
- Después de instalar la versión productiva, el requisito permanente es **install-once**: nunca exigir reinstalar, borrar caché, cerrar Safari ni reiniciar el teléfono para recibir actualizaciones (el mecanismo SW network-first + `/version.json` + banner ya lo cubre).
- La **prueba de recuperación debe cumplir el RPO declarado**; **no prometer pérdida cero** fuera de una sincronización confirmada al object store.

**Hardening posterior (NO M1):** 2+ workers de intake con reinicio rolling (recepción sin blip durante deploy); refresh tokens / revocación individual de chofer; mTLS o basic-auth de Caddy sobre los endpoints de drenaje + allowlist de IP si el Motor gana IP de egreso estable; notificación push al Motor cuando hay envíos nuevos (hoy: pull por timer); alertas "envíos sin drenar > N h" y fallo del job de backup; migración de `schema_version` de `envio.json` con tolerancia doble; PWA estática a Cloudflare Pages (M1-D opcional); `dev-server.js` como perfil de Caddy para compartir config dev/prod.

**Fuera de alcance / separado (no bloquea M1):** SaaS multiempresa; mover OCR/Motor o dataset/catálogos/Drive a la nube; aislamiento por empresa; push en tiempo real a Desktop. Los **7 bloqueantes a–g** de concurrencia del Motor (entrada 2026-09-02) siguen separados y no bloquean M1. **Guía 472624 diferida** (reproceso persistido, pendiente para el PC de casa). El **bloqueo de OCR PaddleOCR CPU en el PC de oficina** (el worker excede el watchdog de 180 s sobre la imagen de 12 MP de 472624; startup ~8 s, inferencia full-res > 9 min; requiere GPU o downscale de resolución, que es cambio de código) sigue fuera de M1.

**Referencia suplementaria (no necesaria para reconstruir):** artifact de diseño con diagrama y tabla de hosting, `https://claude.ai/code/artifact/0fb61d4e-5863-414c-b66b-5f0c1e0b4eee`.

**ESTADO: diseño M1 APROBADO CON CAMBIOS. Sólo documentación. Sin código, sin infraestructura, sin despliegue.**
