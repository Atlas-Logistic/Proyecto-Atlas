...
Diseño integral de consistencia operacional del subsistema Mobile/dataset/decisiones/reporte (inventario completo de escritores reales, modelo de locks, modelo de CAS/versionado, plan por bloques), seguido de implementación en dos fases sobre `atlas_core/` (Motor), más revisión del reproceso persistido idempotente ya existente.

**Reproceso persistido Mobile** (`reprocesar_envio_mobile_persistido`, `atlas_core/mobile.py`): confirmado journal separado del recurso que puede fallar, verificación por huella (nunca restaura ciego), fases explícitas `FASE_PREPARADO`/`FASE_PROCESADO_EN_MEMORIA`/`FASE_DATASET_REEMPLAZADO`/`FASE_DERIVADOS_REGENERADOS`/`FASE_COMPLETADO`, `FASE_COMPLETADO` persistida antes de limpiar el journal, reentrada idempotente sin repetir OCR/B1.

**Fase 1 — base de consistencia:** lock físico único `revalidacion_dataset` unificado en todos los escritores reales del CSV identificados (Mobile normal vía `_procesar_envio_mobile_impl`, `procesamiento_masivo.py` -- flush por lote y `_ejecutar_ia_operacional` con merge de deltas sobre lectura fresca, `reconciliar_incidencias_rut_chofer_documental` con verificación CAS por fila, reproceso persistido, `aplicar_decision_obra`). Lock nuevo `decisiones_pendientes` (constante `NOMBRE_LOCK_DECISIONES_PENDIENTES`, `atlas_core/decisiones_pendientes.py`) para serializar lectura-fresca→merge→dedupe→publicación de `decisiones_pendientes.json`, con patrón wrapper/`_sin_lock` (`generar_artefacto` público con lock, `_generar_artefacto_sin_lock` para llamadores que ya lo sostienen) replicado en los 5 `reconciliar_decisiones_*` de `revalidacion_documental.py` y en `analizar_guias_masivo.py`. Lock por envío `mobile_<envio_id>` (`TIEMPO_EXPIRACION_LOCK_ENVIO_SEGUNDOS = 1800`) sosteniendo la operación COMPLETA de `procesar_envio_mobile`/`reprocesar_envio_mobile_persistido` (incluida OCR); regresión real encontrada y corregida en `RepositorioEnviosMobile.recibir` (colisionaba con reintentos legítimos del cliente Mobile) mediante una vía rápida sin lock para el caso de reintento (`registro_path.is_file()` ya existente). Orden global de adquisición: `mobile_<envio_id>` → `revalidacion_dataset` (ventanas breves) → `decisiones_pendientes` (ventana separada, nunca anidada con el dataset) → locks de catálogo uno a la vez.

**Fase 2 — publicación, recuperación y cierre:** eliminados los rollbacks ciegos de snapshot completo del dataset. `aplicar_decision_obra` (`atlas_core/aplicacion_decisiones.py`): revert de campo por campo con verificación CAS (`_calcular_revert_dataset`/`_revertir_dataset_por_campos` -- relee fresco bajo lock, sólo revierte si los campos siguen siendo los que esta misma operación escribió, si no se abstiene con diagnóstico). Los 6 catálogos de identidad (obras, destinos, vehículos, clientes, evidencia, incidencias documentales) mantienen revert por snapshot completo pero ahora bajo el lock propio de cada catálogo (nuevos: `catalogo_clientes`/`catalogo_destinos`, agregados a `CatalogoClientes`/`CatalogoDestinos` que no tenían ninguno) y con verificación CAS contra un checkpoint (`snapshot_despues_catalogos`) tomado justo antes de la última operación que puede fallar. El dataset, `decisiones_pendientes.json` y `estado_operacion.json` quedaron fuera del bucle de restauración ciega por completo -- sus escritores reales ya son atómicos y seguros de conservar. `reconciliar_estado_derivado` (`atlas_core/reconciliacion_estado_derivado.py`) rediseñado igual: ya no restaura dataset ni `estado_operacion.json` ante fallo, sólo limpia una carpeta de reporte a medio generar. Publicación versionada de reporte + `estado_operacion.json` en los 4 sitios que publican (`revalidar_y_regenerar_reporte`, el bloque de regeneración directa de `aplicar_decision_obra`, `reconciliar_estado_derivado`, `generar_reporte_viajes.py`): huella del dataset capturada antes de generar el reporte, verificada sin cambios después, y sólo entonces se publica `estado_operacion.json` con esa misma huella (`dataset_sha256`); si cambió, el reporte recién generado se descarta y no se publica como vigente. Auditoría final de escritores encontró y corrigió además `analizar_guias_masivo.py` y `generar_reporte_viajes.py` (publicaban sin el patrón protegido).

**Suite:** verificada repetidamente durante la jornada (Fase 1, Fase 2, final). Última corrida completa: **2563 passed, 7 failed** -- 6 failures preexistentes no relacionados (consultas Atlas contra datos reales de `G:\Mi unidad\Atlas`, ver `tests/test_consultas_atlas_e2e.py`/`test_r2_coherencia_revision_accionable.py`/`test_responder_consulta_atlas.py`) más 1 flake de timing puntual (`test_varios_trips_faltantes_se_piden_en_paralelo_no_secuencial`, telemetría paralela, wall-clock -- confirmado como flake al reproducirse limpio 3/3 en aislamiento, módulo no tocado esta jornada). Ninguna regresión funcional atribuible a los cambios de hoy.

**Revisión integral final** (posterior a Fase 2, no ejecutada por el mismo proceso que implementó) encontró 7 bloqueantes reales sin corregir todavía: (a) ventana check→publish entre verificar huella y publicar reporte/`estado_operacion` no es atómica; (b) verificación CAS de rollback de catálogo puede evaluarse fuera del lock o contra snapshot ya obsoleto; (c) `aplicar_decision_obra` puede publicar decisiones desde una lectura de bandeja ya obsoleta; (d) `reconciliar_bandeja_decisiones` tiene el mismo problema que (c); (e) `_validar_csv_existente` escribe el dataset sin pasar por el lock común; (f) `_ejecutar_ia_operacional` puede aplicar su delta sobre una versión ya vieja de los campos, sin CAS por campo en el momento de aplicar; (g) la asociación Mobile puede operar sobre una copia obsoleta del dataset, y la resolución manual de asociación no revalida contra el estado vigente antes de aplicarse. Detalle completo en `docs/HANDOFF_ATLAS.md` (entrada `2026-09-02 — CIERRE DE JORNADA`).

**Git:** 23 archivos (22 modificados + `tests/test_mobile_reproceso_persistido.py` nuevo) en un único commit WIP, `3efdc972220b7bb624efe335bdc95599c9cd6838`, rama `lector-mvp-guia-nueva`, publicado (`git push`, local = remoto, `0/0`). No se tocó `G:\Mi unidad\Atlas`. No se ejecutó la guía real 472624.

**ESTADO: WIP publicado, NO listo para producción.** Siguiente paso: corrección AGRUPADA de los 7 bloqueantes (a-g) como una sola implementación.

## 2026-09-03 — DISEÑO MOBILE PRODUCCIÓN M1 (recepción 24/7) — CIERRE DOCUMENTAL, APROBADO CON CAMBIOS

Revisión de diseño (sin implementar, sin infraestructura, sin tocar `G:\Mi unidad\Atlas`). Objetivo M1: recepción de guías Mobile 24/7 desacoplada del PC de Javier (sin dev-server 8443, backend 8765, IP LAN, certificados autofirmados ni intervención en Safari). El procesamiento OCR/Motor sigue local y posterior.

**Arquitectura aprobada.** `PWA (sin cambios) → intake HTTPS 24/7 (cloud) → almacenamiento durable cloud`. Intake = `crear_servidor(procesar=False)` detrás de Caddy (TLS Let's Encrypt automático, sirve también la PWA + proxy); sólo `POST /api/mobile/login` + `POST /api/mobile/envios` con el contrato intacto (respuesta `estado:"RECIBIDO"`); escribe `operacion/mobile/envios/<envio_id>/{original.<ext>,envio.json}` atómico. `ATLAS_DATA_DIR` a un volumen de bloque; único catálogo presente en el host: `catalogos_privados/usuarios_mobile.json`. El Motor local pasa de servidor a cliente: `drenar_envios_mobile.py` (tarea programada) hace pull por HTTPS y procesa con el pipeline existente (`_procesar_y_revalidar` / `reprocesar_envio_mobile_persistido`). Hosting recomendado: VPS mínimo (Hetzner/DO) + Caddy; alternativa Cloudflare Pages+Worker+R2. Coste piloto ≈ US$6–8/mes.

**Hallazgos de código que condicionan la reconstrucción.** `atlas_core/mobile.py` importa a nivel de módulo (líneas 24-26) `ocr_provider.crear_proveedor_ocr` y `procesamiento_masivo` (`COLUMNAS`, `procesar_archivo`, `escalar_resultado_ia_en_memoria`, `_escribir_filas`); `servidor_mobile.py` importa `revalidacion_documental` en el top. `AutenticadorMobile`/`RepositorioEnviosMobile` viven en `mobile.py` → importar el intake arrastra easyocr/paddle/groq/torch. `RepositorioEnviosMobile.pendientes()` filtra `estado == "REQUIERE_REVISION"` (semántica del Motor, no reutilizable para el intake cloud). `crear_servidor` ya tolera un host pelado (`leer_estado_operacion` → `{}`, `validar_fuente_catalogos(permitir_sin_catalogos=True)`), pero `main()` sí exige `catalogos_privados/usuarios_mobile.json`. `AutenticadorMobile`: token HMAC-SHA256 `cuerpo.firma` base64url, TTL 86400 s, secreto ≥24 chars en `ATLAS_MOBILE_TOKEN_SECRET`. Fast-path idempotente ya presente en `recibir` (si `envio.json` existe → `(registro, False)` sin lock).

**5 cambios obligatorios.** (1) Desacoplar auth+repo+`_multipart`+handlers `/login`/`/envios` de las dependencias OCR (lazy-import detrás de `procesar=True` o módulo liviano aparte) -- prerequisito duro. (2) Lifecycle cloud `RECIBIDO`→`DRENADO` en `envio.json`, `GET /pendientes` filtra `RECIBIDO`, separado del estado operacional. (3) ACK = copia durable local (descarga → verifica contra `envio.json.imagen_sha256` → escribe local atómico → ACK idempotente), nunca "procesado"; el procesamiento local es paso posterior separado con reintento propio. (4) `ATLAS_MOBILE_DRAIN_TOKEN` independiente (≥32 bytes, `hmac.compare_digest`, endpoints 404 si no está seteado, `_ID_SEGURO.fullmatch` en cada parámetro de ruta, sin cross-accept con el token de chofer), `ALLOWED_ORIGIN` al dominio, rate-limit en `/login`, TLS-only+HSTS. (5) Backup a object store R2/B2 con versioning, RPO ≤10 min, volumen de bloque separado con snapshots.

**API de drenaje nueva (activa sólo con `ATLAS_MOBILE_DRAIN_TOKEN`):** `GET /api/mobile/pendientes`, `GET /api/mobile/envios/<id>`, `GET /api/mobile/envios/<id>/imagen`, `POST /api/mobile/envios/<id>/ack`.

**Orden.** M1-A intake lean + hosting (cambio 1 + `--solo-intake` + Dockerfile + Caddyfile + dominio + TLS + `usuarios_mobile.json` + secretos frescos; gate: envío desde datos móviles con Atlas apagado, sobrevive reinicio del host). M1-B drenaje fiable (cambios 2, 3 y drenaje del 4 + `drenar_envios_mobile.py` + tarea programada; gate: Motor 24 h apagado drena todo el backlog, byte-idéntico a subida directa, sin efecto duplicado ante re-descargas). M1-C durabilidad + borde + cutover (cambio 5 + resto del 4 + cabeceras `no-store` en `/version.json` y `service-worker.js` + PWA al dominio y reinstalada una vez + runbook; gate: borrar el volumen y restaurar cumpliendo el RPO declarado, banner de auto-update funcionando desde el origen nuevo).

**Aclaraciones canónicas.** No afirmar "exactly once" -- la garantía es idempotencia / efecto único aunque haya re-descargas por fallos. El paso de la PWA dev por IP LAN al dominio productivo puede requerir UN cutover/reinstalación inicial. Tras instalar la versión productiva el requisito permanente es install-once (nunca reinstalar / borrar caché / cerrar Safari / reiniciar el teléfono para recibir actualizaciones). La prueba de recuperación debe cumplir el RPO declarado, sin prometer pérdida cero fuera de una sincronización confirmada.

**Hardening posterior (no M1):** 2+ workers de intake con reinicio rolling; refresh/revocación individual de token; mTLS o basic-auth de Caddy sobre drenaje + allowlist de IP; push al Motor ante envíos nuevos; alertas de backlog sin drenar y de fallo de backup; migración de `schema_version` con tolerancia doble; PWA a Cloudflare Pages (M1-D opcional).

**Separado, no bloquea M1:** 7 bloqueantes a–g de concurrencia del Motor (entrada 2026-09-02); guía 472624 (diferida, PC de casa); bloqueo de OCR PaddleOCR CPU del PC de oficina (worker excede watchdog de 180 s sobre imagen de 12 MP; startup ~8 s, inferencia full-res > 9 min; requiere GPU o downscale = cambio de código).

**Git:** sólo `docs/` en este commit. Ningún cambio de código ni de infraestructura. `G:\Mi unidad\Atlas` no tocado.

## 2026-09-03 — DISEÑO GEOGRAFÍA G1 (capa geográfica multipaís + catálogo oficial de comunas) — CIERRE DOCUMENTAL, APROBADO

Revisión + diseño (sin implementar, sin copiar datasets al catálogo productivo, sin modificar lógica, sin tocar `G:\Mi unidad\Atlas`). Cruce del subsistema geográfico actual con la investigación `G:\Mi unidad\Atlas\investigacion\geografia_chile_g1\` (catálogo SUBDERE `CUT_2018`, 346 comunas, ya validado 346/346 contra la fuente).

**Subsistema geográfico actual (mapa).** Comuna/territorio: `atlas_core/territorio_chile.py` — snapshot **hardcodeado**, 16 regiones / **345** comunas, adaptado de `jromerof/regiones-chile` (no oficial) con typos parchados a mano; expone `normalizar_comuna()` (exacta / fuzzy difflib ≥0.87 / ambigua / no reconocida), `region_valida()` (mapa de alias), `normalizar_direccion_con_comunas()`; sin provincia, sin códigos CUT, sin polígonos; lo importan sólo `rutas/destino_entrega.py` y `rutas/nominatim.py`. Destino: `rutas/destino_estructurado.py` + `catalogo_destinos.py` (`Destino.comuna`/`region` texto libre; `clave_fisica_destino` por nombres normalizados). Geocodificación: `rutas/openrouteservice.py` (Pelias, `boundary.country=CL`, sólo texto libre) + `rutas/nominatim.py` (respaldo, modo estructurado por parseo frágil). Caché: `rutas/cache_geocodificacion.py`, clave `(proveedor, versión, dirección)` — `version` embebe sólo `pais`. Ruteo: `OpenRouteService.calcular_ruta` (driving-hgv) vía `enriquecimiento_viaje.py` / `destino_entrega.py`. Validación territorial: `_comunas_territorialmente_compatibles` (caso especial literal `"SANTIAGO"`) + `RANGO_LATITUD_RM/LONGITUD_RM` (bounding box RM hardcodeado). Derivados de origen/destino en el dataset: `direccion/localidad/region_entrega`, `estado_entrega`, `planta_origen_*`, `distancia_km`, `duracion_min`, `estado_ruta`, `motivo_ruta` — invariante R2.5 (`CAMPOS_DERIVADOS_RUTA` + `invalidar_derivados_ruta()`) dispara con cambio de planta o dirección.

**Hallazgos que justifican G1:** territorio hardcodeado en código; fuente no oficial; 345 vs 346 oficiales; sin CUT ni nivel provincia (todo indexa por nombre); normalización geográfica fragmentada (`_texto_simple_territorio`, `normalizar_nombre_destino`, `_normalizar_direccion` — sin forma canónica compartida); caché sin territorio (un cambio de catálogo no invalida entradas); supuestos RM/`"SANTIAGO"` dentro del motor.

**Decisión arquitectónica: capa geográfica MULTIPAÍS.** Chile/SUBDERE = primer adaptador/dataset, no el modelo del núcleo. Agregar otro país = adaptador + dataset, nunca rediseñar routing.

**Núcleo genérico `atlas_core/geografia/` (neutro de país):**
- `UnidadAdministrativa`: `codigo_pais` · `nivel` (ordinal 1..N, nunca conteo fijo) · `codigo` (oficial estable, opaco — el motor no lo parsea) · `codigo_padre` (jerarquía) · `nombre_canonico` · `nombre_normalizado` (única forma canónica) · `aliases` · `geometria` (opcional, por `codigo`; G2) · `metadata` (extras por país).
- `ResultadoNormalizacion` (reemplaza `ResultadoNormalizacionComuna`): `estado` (EXACTA/NORMALIZADA_SEGURA/AMBIGUA/NO_RECONOCIDA — sin cambios) · `valor_original` · `unidad` · `similitud` · `unidad_de_nivel(n)` (sube por `codigo_padre` — reemplaza el campo `region`).
- Protocolo `GeografiaPais`: `codigo_pais` (alfa-2/alfa-3) · `niveles` · `nivel_geocodificable` · `nivel_region_geocodificacion` · `normalizar(texto, nivel=None)` · `normalizar_direccion(texto)` · `buscar_por_codigo(codigo)` · `parametros_geocodificacion(unidad)` · `compatibilidad_territorial(a, b)`.
- `cargar_geografia(codigo_pais)` — `PAIS_OPERACION_PREDETERMINADO` elige el default.
- Motor de normalización en el núcleo como defaults (estados + umbrales fuzzy `0.87` / `0.06` / long≥`4`); índice de unidades y lista de palabras estructurales inyectados por el adaptador.

**Adaptador Chile `geografia/cl.py`:** etiquetas región/provincia/comuna; 16/56/346; `nivel_geocodificable = comuna`; CUT + `abreviatura_region`; alias de región (`"RM"`…) → `aliases` de NIVEL_1; vocabulario español de dirección; "Santiago área metropolitana" → alias, no `if`; bounding box RM → `rango_operacion` del contexto. Dataset `geografia_chile.json` en `catalogos_privados/` (patrón portable) + sidecar de procedencia; **reemplaza el snapshot no oficial de 345**.

**Consecuencias:**
- **Una autoridad canónica de normalización desde G1**: `catalogo_destinos.normalizar_nombre_destino` y `cache_geocodificacion._normalizar_direccion` **delegan** en el adaptador (no reescritura); fundir los tres cuerpos en uno solo = posterior.
- Geocodificación estructurada: adaptadores consumen `parametros_geocodificacion(unidad)`, nunca "comuna"/"región"; texto libre de fallback.
- Validación post-geocodificación: `compatibilidad_territorial` por `codigo` + ancestro; reemplaza `_comunas_territorialmente_compatibles` y el literal `"SANTIAGO"`.
- Caché: `codigo_unidad` genérico en la clave + subir `version`.
- Routing (`calcular_ruta` driving-hgv, `ServicioRutas`, `RepositorioRutas`): **sin cambios** (sólo coordenadas); `OpenRouteService(pais=…)`/`NominatimGeocoder(pais=…)` ya reciben ISO de país.
- Compatibilidad hacia atrás: `comuna`/`region`/`codigo_comuna` permanecen (dataset y `Destino`), poblados desde `UnidadAdministrativa`; no definen el modelo interno.
- Invalidación de derivados: cambio territorial del destino (unidad geocodificable o ancestro región) → `invalidar_derivados_ruta()` + descarte de caché stale.
- Polígonos = G2 (test geométrico punto-en-unidad; depende de GIS + reproyección EPSG:5360→4326, fuera del entorno).

**Colombia — sólo valida el contrato (sin investigar ni implementar):** departamento (NIVEL_1) + municipio/distrito (NIVEL_2, `nivel_geocodificable=2`); DIVIPOLA como `codigo`; `metadata.tipo` = MUNICIPIO/DISTRITO; Bogotá D.C. = unidad NIVEL_2 con `codigo_padre` a NIVEL_1 homónima (sin `if`); distinto número de niveles resuelto con ordinales + `nivel_geocodificable`. **Agregar otro país = adaptador + dataset; routing/núcleo/caché/validación no cambian.**
